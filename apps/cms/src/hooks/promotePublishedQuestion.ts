...
export const promotePublishedQuestion: CollectionAfterChangeHook = async ({ doc, req }) => {
  ...
  const body = {
    ...
    program_key: String(full.programKey ?? ''),
    path_key: String(full.pathKey ?? ''),
    domain_key: String(full.domainKey ?? ''),
    capability_key: String(full.capabilityKey ?? ''),
    mapping_state: String(full.mappingState ?? ''),
