...
"${compose[@]}" exec -T postgres psql -v ON_ERROR_STOP=1 -U question_brain -d question_brain \
  < "${repo_root}/db/migrations/0018_question_capability_bindings.sql"

echo "question-brain capability binding migration: applied 0018_question_capability_bindings.sql"
