...
"${compose[@]}" exec -T postgres psql -v ON_ERROR_STOP=1 -U question_brain -d question_brain \
  < "${repo_root}/db/migrations/0019_capability_alias_supersession_review.sql"

echo "question-brain capability alias review migration: applied 0019_capability_alias_supersession_review.sql"
